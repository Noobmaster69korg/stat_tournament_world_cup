...
# --- 1. SCRAPER LOGIC (Resilient BeautifulSoup Parser) ---
def scrape_cricinfo(url, discipline):
    try:
        ...
        if discipline == "batting":
            # Cricinfo Batting: Player(0), Span(1), Runs(5), Ave(7), SR(9)
            df = raw_df[[0, 1, 5, 7, 9]].copy()
            df.columns = ['Player', 'Season', 'Runs', 'Ave', 'SR']
        else:
            # Cricinfo Bowling: Player(0), Span(1), Wkts(6), Ave(8), Econ(9)
...
if st.sidebar.button("🚀 Build Database"):
    if b_link and w_link:
        with st.spinner("Scraping..."):
            b_df, w_df = scrape_cricinfo(b_link, "batting"), scrape_cricinfo(w_link, "bowling")
...
if st.session_state.nav_choice == "Batting Milestones":
    ...
    q = f"WITH Base AS (SELECT Player, Season as Year, Runs, Ave as Average, SR as Strike_Rate, (CASE WHEN Runs > {tr} THEN 1 ELSE 0 END + CASE WHEN Ave > {ta} THEN 1 ELSE 0 END + CASE WHEN SR > {ts} THEN 1 ELSE 0 END) as WinsA, (CASE WHEN Runs = {tr} THEN 1 ELSE 0 END + CASE WHEN Ave = {ta} THEN 1 ELSE 0 END + CASE WHEN SR = {ts} THEN 1 ELSE 0 END) as TiesA, (CASE WHEN Runs < {tr} THEN 1 ELSE 0 END + CASE WHEN Ave < {ta} THEN 1 ELSE 0 END + CASE WHEN SR < {ts} THEN 1 ELSE 0 END) as LossesA, (CASE WHEN Runs > {trb} THEN 1 ELSE 0 END + CASE WHEN Ave > {tab} THEN 1 ELSE 0 END + CASE WHEN SR > {tsb} THEN 1 ELSE 0 END) as WinsB, (CASE WHEN Runs = {trb} THEN 1 ELSE 0 END + CASE WHEN Ave = {tab} THEN 1 ELSE 0 END + CASE WHEN SR = {tsb} THEN 1 ELSE 0 END) as TiesB FROM batting) SELECT * FROM Base WHERE (WinsA + TiesA) >= 2 {'AND (WinsB + TiesB) >= 2' if 'A and B' in f_mode else ''} ORDER BY WinsA DESC, Runs DESC"
...
# --- TAB 5: SQUAD ---
    ...
    if st.session_state.squad_a and st.session_state.squad_b:
        ...
        if sub == "Individual Benchmarking":
            ...
            with c2:
                st.write("B vs A")
                qb = f"SELECT A.Player, A.Season, (SELECT COUNT(*) FROM {t} B WHERE B.Player IN {a_l}) as TR, (SELECT COUNT(*) FROM {t} B WHERE B.Player IN {a_l} AND {win} >= 2) as WC, (SELECT COUNT(*) FROM {t} B WHERE B.Player IN {a_l} AND {loss} >= 2) as LC FROM {t} A WHERE A.Player IN {b_l}"
...
# --- TAB 6: FORMAT ANALYSIS ---
    ...
    sub_tab_6 = st.radio("Select Feature:", ["🛡️ Unbeatable Combinations", "🔍 Group Killers"], horizontal=True)
    disc_f = st.radio("Choose Discipline:", ["Batting", "Bowling"], horizontal=True, key="disc_fa_main")
    t = disc_f.lower()
    raw_p = sorted(pd.read_sql(f"SELECT DISTINCT Player FROM {t}", conn)['Player'].tolist())

    if sub_tab_6 == "🛡️ Unbeatable Combinations":
        cx1, cx2 = st.columns(2)
        with cx1: omit_cand = st.multiselect("Omit from Pool:", raw_p, key="om1")
        with cx2: omit_kill = st.multiselect("Omit from Killers:", raw_p, key="om2")
        k_v = st.radio("Size (K):", [1, 2, 3], horizontal=True)
        if st.button("🚀 Find Unbeatable"):
            with st.spinner("Analyzing..."):
                df_f = pd.read_sql(f"SELECT * FROM {t}", conn)
                df_k = df_f[~df_f['Player'].isin(omit_kill)]
                cand = [p for p in raw_p if p not in omit_cand]
                
                # Check if candidates list is empty to prevent crash
                if not cand:
                    st.error("No players left in the pool. Adjust Omissions.")
                else:
                    kill_sets = []
                    ...
                    if res:
                        ...
                        st.error("No combination is unbeatable against the filtered database.")
    else:
                ...
                with st.spinner("Searching..."):
                    ...
                    for _, rx in df_full.iterrows():
                        ...
                        for pn in target:
                            ...
                            if not pb: entire = False; break
                        if entire: kl.append(rx)
                    if kl:
                        st.success(f"Found {len(kl)} killers!")
                        st.dataframe(pd.DataFrame(kl), use_container_width=True, hide_index=True)
                    else:
                        st.error("No single year beats this entire group.")
