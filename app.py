...
# --- 1. NEW MANUAL SCRAPER (Bypasses lxml error) ---
def scrape_cricinfo(url, discipline):
    try:
        ...
        # Cricinfo stats tables usually have the class 'engineTable'
        table = soup.find_all('table', class_='engineTable')
        
        # We want the table that has 'Player' in the header
        target_table = None
        for t in table:
        ...
        if thead:
            headers_list = [th.get_text().strip() for th in thead.find_all('th')]
        else:
            headers_list = [th.get_text().strip() for th in target_table.find_all('tr')[0].find_all('td')]
        ...
        st.sidebar.error(f"Manual Scrape Error: {e}")
        return None
...
if st.sidebar.button("🚀 Build Database"):
    if bat_url and bowl_url:
        with st.spinner("Scraping with BeautifulSoup..."):
            ...
            if b_df is not None and w_df is not None:
                ...
                st.sidebar.error("Scrape failed. Check links.")
...
# TAB 5: SQUAD
    ...
    with st.expander("Manage Squads"):
        c1, c2 = st.columns(2)
        if c1.button("Clear"): st.session_state.squad_a, st.session_state.squad_b = [], []; st.rerun()
    ...
    if st.session_state.squad_a and st.session_state.squad_b:
        ...
        if sub == "Benchmarking":
            d_dir = st.radio("Direction:", ["A to B", "B to A"], horizontal=True)
            ...
            with c2:
                st.write("B vs A")
                q = f"SELECT A.Player, A.Season, (SELECT COUNT(*) FROM {t} B WHERE B.Player IN {a_l}) as TR, (SELECT COUNT(*) FROM {t} B WHERE B.Player IN {a_l} AND {win} >= 2) as WC, (SELECT COUNT(*) FROM {t} B WHERE {a_l} AND {loss} >= 2) as LC FROM {t} A WHERE A.Player IN {b_l}"
                df = pd.read_sql(q, conn); df['Wins'], df['Losses'], df['Ties'] = df.apply(lambda r: fmt(r['WC'], r['TR']), axis=1), df.apply(lambda r: fmt(r['LC'], r['TR']), axis=1), df.apply(lambda r: fmt(r['TR']-r['WC']-r['LC'], r['TR']), axis=1)
                st.dataframe(df[['Player', 'Season', 'Wins', 'Losses', 'Ties']], hide_index=True)
    ...
    if "Unbeatable" in sub6:
        ...
        if st.button("Find Combinations"):
            with st.spinner("Analyzing..."):
                df_all = pd.read_sql(f"SELECT * FROM {t}", conn); df_k = df_all[~df_all['Player'].isin(o2)]; cand = [p for p in pl if p not in o1]; k_sets = []
                for _, rx in df_k.iterrows():
                    beaten = set()
                    for pn in cand:
                        py = df_all[df_all['Player']==pn]; b = 0
