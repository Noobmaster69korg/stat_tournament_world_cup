...
if st.session_state.nav_choice == "Batting Milestones":
    ...
    with col_a:
        ...
        # We use st.session_state.KEY as the value. 
        # Widget updates session_state automatically upon change.
        tr1a = st.number_input("Min Runs (A)", value=st.session_state.bat_runs_a, key="bat_runs_a_input")
        ta1a = st.number_input("Min Average (A)", value=st.session_state.bat_avg_a, key="bat_avg_a_input")
        ts1a = st.number_input("Min Strike Rate (A)", value=st.session_state.bat_sr_a, key="bat_sr_a_input")
        # Update session state manually to ensure Tab 4 can pull latest user edits
...
# --- TAB 2: BOWLING ---
    ...
    with col_c:
        st.subheader("Set A (Primary)")
        twa = st.number_input("Min Wickets (A)", value=st.session_state.bowl_w_a, key="bowl_w_a_input")
        taa = st.number_input("Max Average (A)", value=st.session_state.bowl_a_a, key="bowl_a_a_input")
        tea = st.number_input("Max Economy (A)", value=st.session_state.bowl_e_a, key="bowl_e_a_input")
...
# --- TAB 4: PLAYER DETAILS ---
    ...
    if target_player:
        ...
        df_b = pd.read_sql(b_prof_q, conn)
        if not df_b.empty:
            ...
            # Restored display columns
            view_cols = ['Season', 'Runs', 'Ave', 'SR', 'Wins (Percentile)', 'Losses', 'Ties', 'Clean_Loss']
            evt_b = st.dataframe(df_b[view_cols], use_container_width=True, hide_index=True, on_select="rerun", selection_mode="single-row", key="sel_b")
        ...
        df_w = pd.read_sql(w_prof_q, conn)
        if not df_w.empty:
            ...
            # Restored display columns
            view_cols_w = ['Season', 'Wkts', 'Ave', 'Econ', 'Wins (Percentile)', 'Losses', 'Ties', 'Clean_Loss']
            evt_w = st.dataframe(df_w[view_cols_w], use_container_width=True, hide_index=True, on_select="rerun", selection_mode="single-row", key="sel_w")
...
conn.close()    if w == 1 and t == 1 and l == 1: return "⚖️ Beat 1 category, Tied 1, Lost 1 (Tie Case)"
    if w == 0 and t == 3: return "🎯 Tied all 3 categories"
    if w == 0 and t == 2 and l == 1: return "🤏 Tied 2 categories, Lost 1 category"
    return "Other"
