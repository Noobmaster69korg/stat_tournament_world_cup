...
def get_db_connection():
    # If custom DB is active and file exists, use it
...
def scrape_full_cricinfo(base_url, discipline):
    ...
    while True:
        ...
        try:
            ...
            # Look for all tables and find the one containing player stats
            ...
            # Map headers on the first page
            if page == 1:
                header_row = target_table.find_all('th')
                if not header_row: header_row = target_table.find('tr').find_all('td')
                header_texts = [th.get_text().strip() for th in header_row]
                col_map = {txt: i for i, txt in enumerate(header_texts)}
            ...
            page += 1
            progress_bar.progress(min(page / 20, 1.0)) # Visual progress
            if len(rows) < 50 or page > 40: break 
        except: break
...
if "use_scraped_db" not in st.session_state:
    st.session_state.use_scraped_db = False

# Permanent Status Display
if st.session_state.use_scraped_db:
    st.sidebar.success("🟢 Status: Live Scraped Data Active")
else:
    st.sidebar.info("🏠 Status: Default Local Database Active")

bat_link = st.sidebar.text_input("ESPN Batting Link")
bowl_link = st.sidebar.text_input("ESPN Bowling Link")

c_sb1, c_sb2 = st.sidebar.columns(2)
if c_sb1.button("🚀 Build DB"):
    if bat_link and bowl_link:
        with st.spinner("Scraping Cricinfo (Page by Page)..."):
            ...
            if b_df is not None and w_df is not None:
                ...
                st.rerun()
            else: st.sidebar.error("Failed to scrape. Check URL.")
...
for k in benchmark_keys:
    if k not in st.session_state:
        st.session_state[k] = 300 if "runs" in k else 40.0 if "avg" in k else 90.0 if "sr" in k else 15 if "w" in k else 25.0 if "a_a" in k else 5.0
...
if st.session_state.nav_choice == "Batting Milestones":
    ...
    with c2:
        tr1b = st.number_input("Min Runs (B)", 500); ta1b = st.number_input("Min Average (B)", 50.0); ts1b = st.number_input("Min SR (B)", 100.0)
...
# --- TAB 2: BOWLING ---
    ...
    with c2:
        twb = st.number_input("Min Wickets (B)", 20); tab = st.number_input("Max Avg (B)", 20.0); teb = st.number_input("Max Econ (B)", 4.5)
...
# --- TAB 5: SQUAD COMPARISON ---
    ...
    with st.expander("🛠️ Manage Squads"):
        ...
        load = st.text_input("📥 Paste Code:"); ( (d := json.loads(load)), st.session_state.__setitem__('squad_a', d.get('a', [])), st.session_state.__setitem__('squad_b', d.get('b', [])), st.rerun() ) if st.button("🔄 Go") and load else None
    ...
    if st.session_state.squad_a and st.session_state.squad_b:
        ...
        if sub == "Individual Benchmark":
            ...
            a_l, b_l = "('" + "','".join(st.session_state.squad_a) + "')", "('" + "','".join(st.session_state.squad_b) + "')"
...
# --- TAB 6: FORMAT ANALYSIS ---
    ...
    if "Unbeatable" in sub6:
        ...
        with cx1: o1 = st.multiselect("Omit Pool:", pl, key="om1"); with cx2: o2 = st.multiselect("Omit Check:", pl, key="om2")
