...
def get_db_connection():
    """Returns a connection to either the scraped DB or the local DB."""
...
def get_profile_label(w, t, l):
    labels = {
        (3, 0, 0): "🏆 Beat all 3 categories",
        (2, 1, 0): "⭐ Beat 2, Tied 1",
        (2, 0, 1): "✅ Beat 2, Lost 1"
    }
    return labels.get((w, t, l), "Other combinations")
...
if col_sb1.button("🚀 Build DB"):
    if bat_link and bowl_link:
        with st.spinner("Scraping..."):
            b_df, w_df = scrape_cricinfo(bat_link, "batting"), scrape_cricinfo(bowl_link, "bowling")
            if b_df is not None and w_df is not None:
                # Save to a physical file to prevent ProgrammingErrors
...
if col_sb2.button("🗑️ Reset to Local"):
    st.session_state.use_scraped_db = False
    if os.path.exists(SCRAPED_DB_FILE): os.remove(SCRAPED_DB_FILE)
    st.rerun()

st.sidebar.write("Status: " + ("🟢 Scraped Data" if st.session_state.use_scraped_db else "🏠 Local DB"))
...
if st.session_state.nav_choice == "Batting Milestones":
    ...
    with c2:
        tr1b = st.number_input("Min Runs (B)", 500)
        ta1b = st.number_input("Min Average (B)", 50.0)
        ts1b = st.number_input("Min Strike Rate (B)", 100.0)
        ...
        if st.button("🔎 Find Killers"):
            if target:
                ...
                for _, rx in df_full.iterrows():
                    ...
                    if e: kl.append(rx)
                if kl: st.success(f"Found {len(kl)} killers!"); st.dataframe(pd.DataFrame(kl), use_container_width=True, hide_index=True)
