            ...
            for tr in rows:
                ...
                try:
                    ...
                    if discipline == "batting":
                        p_stat1, p_stat2, p_stat3 = cells[col_map['Runs']].get_text(), cells[col_map['Ave']].get_text(), cells[col_map['SR']].get_text()
                    else:
                        p_stat1, p_stat2, p_stat3 = cells[col_map['Wkts']].get_text(), cells[col_map['Ave']].get_text(), cells[col_map['Econ']].get_text()
...
if cs1.button("🚀 Build DB"):
    if bat_l and bowl_l:
        with st.spinner("Scraping..."):
            b_df, w_df = scrape_full_cricinfo(bat_l, "batting"), scrape_full_cricinfo(bowl_l, "bowling")
...
# TAB 5: SQUAD COMPARISON (FIXED)
    ...
    if st.session_state.squad_a and st.session_state.squad_b:
        ...
        if sub == "Individual Benchmark":
            d_dir = st.radio("Direction:", ["Squad A is Benchmark", "Squad B is Benchmark"], horizontal=True)
            src, trg = (st.session_state.squad_a, st.session_state.squad_b) if "A is" in d_dir else (st.session_state.squad_b, st.session_state.squad_a)
            # DYNAMIC KEY FOR DROPDOWN
            ...
            with c2:
                ...
                q_b = f"SELECT A.Player, A.Season, (SELECT COUNT(*) FROM {t_disc} B WHERE B.Player IN {a_l}) as TR, (SELECT COUNT(*) FROM {t_disc} B WHERE B.Player IN {a_l} AND {win_l} >= 2) as WC, (SELECT COUNT(*) FROM {t_disc} B WHERE B.Player IN {a_l} AND {loss_l} >= 2) as LC FROM {t_disc} A WHERE A.Player IN {b_l}"
